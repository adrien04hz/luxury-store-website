...
export default function HomeNav() {
    const { count } = useCart();
    const n = count;
    const [ logoSrc, setLogoSrc ] = useState('/images/logo_white.png');

    useEffect( () => {
        const $header = document.getElementById("homebar");
        const onscroll = () => {
            if( window.scrollY > 250){
                $header?.classList.add('fondo2');
                setLogoSrc('/images/logo_black.png');
            }else{
                $header?.classList.remove('fondo2');
    ...
    return (
        <header id="homebar" className="flex justify-between items-center px-10 top-0 w-full h-16 fixed z-10 text-white homeHeader">
